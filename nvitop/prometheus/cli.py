...
def doUpdateMetrics():
    ...
    for key in sm_util:
        ...
        gpu_index = key[0]
        util = sm_util[key]
        gpu_pid_sm_util.labels("500ms", ip_addr, gpu_index, pid).set(util)
    ...
    for key in mem_used:
        ...
        used = mem_used[key]
        gpu_pid_mem_total.labels("500ms", ip_addr, gpu_index, pid).set(total)
        gpu_pid_mem_used.labels("500ms", ip_addr, gpu_index, pid).set(used)
    registry = newRegistry
